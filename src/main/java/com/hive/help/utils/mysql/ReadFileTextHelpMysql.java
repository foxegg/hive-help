...
public class ReadFileTextHelpMysql {
    ...
    private static void outFile(StringBuilder sb,List dataList){
        ...
        String outPath = nowPath.replace("/logs/","/logs_out_sql/");
    ...
    public static void initText(String text) throws ParseException {
        ...
        if (dataId.equals(DateType.LOCATION.getCode())) {//已保存
            ...
            if(appLog.getReport()!=null){
                for(AppReport appReport:appLog.getReport()){
                    ...
                    try{
                        if(appReport.getFirstInstallTime()!=null)
                            app.setFirstInstallTime(sf.parse(appReport.getFirstInstallTime()).getTime()/1000);
                        if(appReport.getLastUpdateTime()!=null)
                            app.setLastUpdateTime(sf.parse(appReport.getLastUpdateTime()).getTime()/1000);
                    }catch (Exception e){
        ...
        }else if (dataId.equals(DateType.RISKSMS002.getCode())) {
            RiskSms002Log riscSms002Log = JSONObject.parseObject(text, RiskSms002Log.class);
            RiskSms002 riskSms002 = new RiskSms002();
            BeanUtils.copyProperties(riscSms002Log,riskSms002);
            BeanUtils.copyProperties(riscSms002Log.getReport(),riskSms002);
            if(ADD_APP){
                DateType.RISKSMS002.getData().add(riskSms002);
            }
            checkAndInsert(DateType.RISKSMS002.getData());
        }
